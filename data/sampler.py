...
def _pairwise_sampling_v2(user_pos_dict, num_samples, num_item):
    ...
    for user, pos_len in user_pos_len.items():
        try:
            ...
            print(f'Sampling error for user {user}: {e}')

    # Filter to only users that have both pos and neg samples
    valid_users = [u for u in users_list if u in user_pos_sample and u in user_neg_sample]
    pos_items_list = [user_pos_sample[u].pop() for u in valid_users]
    neg_items_list = [user_neg_sample[u].pop() for u in valid_users]

    return valid_users, pos_items_list, neg_items_list
