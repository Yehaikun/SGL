...
class _LightGCN(nn.Module):
    def __init__(self, num_users, num_items, embed_dim, norm_adj, n_layers, mask_ratio=0.0):
    ...
    def forward(self, sub_graph1, sub_graph2, users, items, neg_items):
        ...
        if self.mask_ratio > 0:
            fm = torch.rand(user_embeddings.shape[1], device=user_embeddings.device) > self.mask_ratio
            u3 = F.normalize(user_embeddings * fm.float(), dim=1)
            i3 = F.normalize(item_embeddings * fm.float(), dim=1)
        else:
            u3, i3 = user_embeddings1, item_embeddings1
        ...
        return sup_logits, ssl_logits_user, ssl_logits_item, u3, i3
...
class SGL(AbstractRecommender):
    def __init__(self, config):
        ...
        self.lightgcn = _LightGCN(self.num_users, self.num_items, self.emb_size,
                                  adj_matrix, self.n_layers, mask_ratio=self.mask_ratio).to(self.device)
        if self.pretrain_flag:
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for bat_users, bat_pos_items, bat_neg_items in data_iter:
                ...
                sup_logits, ssl_logits_user, ssl_logits_item, u3_all, i3_all = self.lightgcn(
                ...
                if self.mask_ratio > 0:
                    # Get view1 embeddings for this batch (reuse norm_adj forward)
                    u1_all, i1_all = self.lightgcn._forward_gcn(self.lightgcn.norm_adj)
                    u1_all = F.normalize(u1_all, dim=1)
                    i1_all = F.normalize(i1_all, dim=1)
                    u1_batch = F.embedding(bat_users, u1_all)
                    i1_batch = F.embedding(bat_pos_items, i1_all)
                    u3_batch = F.embedding(bat_users, u3_all)
                    i3_batch = F.embedding(bat_pos_items, i3_all)
                    cu3 = torch.logsumexp((torch.matmul(u1_batch, u3_all.T) - inner_product(u1_batch, u3_batch)[:,None]) / self.ssl_temp, dim=1)
                    ci3 = torch.logsumexp((torch.matmul(i1_batch, i3_all.T) - inner_product(i1_batch, i3_batch)[:,None]) / self.ssl_temp, dim=1)
                    infonce_loss += torch.sum(cu3 + ci3)
