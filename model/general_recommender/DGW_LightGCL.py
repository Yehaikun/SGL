...
class DGW_LightGCL(AbstractRecommender):
    def __init__(self, config):
        ...
        assert self.ssl_aug_type in ['nd','ed', 'rw', 'svd']
        ...
        self.svd_q = int(config["svd_q"]) if "svd_q" in config else 5
        ...
        # === DGW-LightGCL: Precompute SVD low-rank matrix ===
        self.svd_aug_adj = self.create_adj_mat(is_subgraph=True, aug_type='svd')
        self.svd_aug_matrix = sp_mat_to_sp_tensor(self.svd_aug_adj).to(self.device)
    ...
    def create_adj_mat(self, is_subgraph=False, aug_type='ed'):
        ...
        if is_subgraph and self.ssl_ratio > 0:
            ...
            if aug_type == 'svd':
                ...
                adj_for_svd = sp.csr_matrix((np.ones_like(users_np, dtype=np.float32), (users_np, items_np)), shape=(self.num_users, self.num_items))
                adj_for_svd = adj_for_svd + adj_for_svd.T
                try:
                    U, S, Vt = svds(adj_for_svd, k=self.svd_q)
                    svd_recon = U @ np.diag(S) @ Vt
                    tmp_adj = sp.csr_matrix((svd_recon[users_np, items_np], (users_np, items_np+self.num_users)), shape=(n_nodes, n_nodes))
                except Exception:
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            elif self.ssl_aug_type == 'svd':
                # Reuse precomputed SVD matrix (no per-epoch recomputation)
                sub_graph1 = self.svd_aug_matrix
                sub_graph2 = self.svd_aug_matrix
