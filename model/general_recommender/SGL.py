...
class SGL(AbstractRecommender):
    def __init__(self, config):
        ...
        try:
            self.hard_alpha = config["hard_alpha"]
        except:
            self.hard_alpha = 0.0
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for bat_users, bat_pos_items, bat_neg_items in data_iter:
                ...
                # InfoNCE Loss
                if self.hard_alpha > 0:
                    logits_u = ssl_logits_user / self.ssl_temp
                    hardness_u = 1.0 + self.hard_alpha * torch.sigmoid(logits_u)
                    clogits_user = torch.logsumexp(logits_u * hardness_u, dim=1)
                else:
                    clogits_user = torch.logsumexp(ssl_logits_user / self.ssl_temp, dim=1)
                if self.hard_alpha > 0:
                    logits_i = ssl_logits_item / self.ssl_temp
                    hardness_i = 1.0 + self.hard_alpha * torch.sigmoid(logits_i)
                    clogits_item = torch.logsumexp(logits_i * hardness_i, dim=1)
                else:
